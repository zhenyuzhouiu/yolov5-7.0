# Based on horizontal bounding box loss, add oriented bounding box loss
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                pxy, pwh, _, pcls, pangle = pi[b, a, gj, gi].split((2, 2, 1, self.nc, 180), 1)  # target-subset of predictions
                ...
                # Rotated Angle
                ta = tangle[i].type(pi.dtype).to(self.device)
                langle += self.BBCEangle(pangle, ta)
    ...
    def build_targets(self, p, targets):
        ...
        :return: indices.append((b, a, gj.clamp_(0, shape[2] - 1), gi.clamp_(0, shape[3] - 1)))  # image, anchor, grid
                 ...
                 tangle.append(csl_a)
        """
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h, 180) with shape [nt, 186]
        # na = 3
        ...
        for i in range(self.nl):
            ...
            # Define
            b, c = t[:, :2].long().T  # long int (image, class)
            a = t[:, -1].long().view(-1)  # anchors
            gxy = t[:, 2:4]  # grid xy
            gwh = t[:, 4:6]  # grid wh
            csl_a = t[:, 6:-1]  # circular smooth angle label
            ...
            anch.append(anchors[a])  # anchors
            tcls.append(c)  # class
            tangle.append(csl_a)  # circular gaussian smooth label
