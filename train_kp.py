...
import val_kp as validate
from models.experimental import attempt_load
# only use the small object head
from models.yolo_kp import Model
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
# from utils.dataloaders import create_dataloader
from utils.dataloaders_kp import create_dataloader
from utils.downloads import attempt_download, is_url
from utils.general_kp import (LOGGER, TQDM_BAR_FORMAT, check_amp, check_dataset, check_file, check_git_info,
...
from utils.loss_kp import ComputeLoss
# from utils.metrics import fitness
from utils.metrics_kp import fitness
# from utils.plots import plot_evolve
from utils.plots_kp import plot_evolve
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    """
    :param hyp: opt.hyp hyperparameter
    :param opt: opt from parser.parse_args()
    :param device: gpu device number
    :param callbacks: for register_hook()
    :return: results = (0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    """

    # ===================================== initial hyperparameter, loggers, directories, run settings, config
    ...
    # ========================================== model, freeze, check image size, batch size, optimizer, scheduler
    # Model
    check_suffix(weights, '.pt')  # check weights
    pretrained = weights.endswith('.pt')  # if weights end with pt, pretrained = True
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
    ...
    amp = check_amp(model)  # check AMP automatic mixed precision
    ...
    # EMA exponential moving average (maintain moving averages of the trained parameters)
    ...
    # DP mode distributed parallel
    # DDP is faster and more flexible than DP distributed data parallel
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                # Linear interpolate with given discrete data points (xp, fp), evaluated at x.
            ...
            # Multi-scale uses different image size (image scale factor) for training
            if opt.multi_scale:
            ...
            # Only arrive the accumulation number, the weights will be updated
        ...
        # scheduler will update by epoch step not by batch step
        scheduler.step()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/yolov5m-kp.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/fingernail-kp.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.fingernail-kp.yaml', help='hyperparameters path')
